		...
		try (BufferedReader lector = Files.newBufferedReader(archivoEntrada,
			...
			while ((linea = lector.readLine()) != null) {
				...
				if (linea.charAt(0)=='(' && linea.contains(")")) {
					...
					irA.put(linea, new Integer(numeroDeLineas));	// Agregamos el número de línea bajo la clave del nombre del símbolo
			...
			while ((linea = lector.readLine()) != null) {
				...
				if (linea.charAt(0)=='@') {	// A-Instruction
					...
					if (coincidencia.find() && coincidencia.start() == 0) {	
						...
						esSimbolo = false;	// Reseteamos la variable que nos muestra si es un símbolo predefinido	
					// Vemos si es uno de los símbolos predefinidos, en cuyo caso lo agregamos
